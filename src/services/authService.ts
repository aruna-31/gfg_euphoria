...
export const DEMO_CREDENTIALS = [
  ...
  { label: 'Jury / Evaluator', email: 'raman@gfgkare.in', password: 'jury123', role: 'EVALUATOR' },
...
class AuthService {
  ...
  public async login(email: string, pass: string): Promise<User> {
    ...
    const isEval = (await evaluatorService.getAllEvaluators()).some((e) => e.email.toLowerCase() === clean);
    if (isEval) {
  ...
  public async loginAsLeader(email: string, _pass: string): Promise<User> {
    ...
    const team = await teamService.getTeamByLeaderEmail(cleanEmail);
    if (!team) {
    ...
    this.assertPassword(cleanEmail, _pass, 'LEADER', team.accessPassword);
  ...
  public async loginAsEvaluator(email: string, _pass: string): Promise<User> {
    ...
    const evaluator = await evaluatorService.getAllEvaluators().then((evaluators) =>
      evaluators.find((e) => e.email.toLowerCase() === cleanEmail)
    );
    if (!evaluator) {
      throw new Error(`Jury credentials not found for: "${cleanEmail}". Check with Hackathon Operations.`);
    }

    this.assertPassword(cleanEmail, _pass, 'EVALUATOR', evaluator.accessPassword);
